...
const ICONS = {
  ...
  orders:     'M20.25 7.5l-.625 10.632a2.25 2.25 0 0 1-2.247 2.118H6.622a2.25 2.25 0 0 1-2.247-2.118L3.75 7.5M10 11.25h4M3.375 7.5h17.25c.621 0 1.125-.504 1.125-1.125v-1.5c0-.621-.504-1.125-1.125-1.125H3.375c-.621 0-1.125.504-1.125 1.125v1.5c0 .621.504 1.125 1.125 1.125Z',
  ...
  products:   'M3.75 6A2.25 2.25 0 0 1 6 3.75h2.25A2.25 2.25 0 0 1 10.5 6v2.25a2.25 2.25 0 0 1-2.25 2.25H6a2.25 2.25 0 0 1-2.25-2.25V6ZM3.75 15.75A2.25 2.25 0 0 1 6 13.5h2.25a2.25 2.25 0 0 1 2.25 2.25V18a2.25 2.25 0 0 1-2.25 2.25H6A2.25 2.25 0 0 1 3.75 18v-2.25ZM13.5 6a2.25 2.25 0 0 1 2.25-2.25H18A2.25 2.25 0 0 1 20.25 6v2.25A2.25 2.25 0 0 1 18 10.5h-2.25a2.25 2.25 0 0 1-2.25-2.25V6ZM13.5 15.75a2.25 2.25 0 0 1 2.25-2.25H18a2.25 2.25 0 0 1 2.25 2.25V18A2.25 2.25 0 0 1 18 20.25h-2.25A2.25 2.25 0 0 1 13.5 18v-2.25Z',
  ingredient: 'M9.75 3.104v5.714a2.25 2.25 0 0 1-.659 1.591L5 14.5M9.75 3.104c-.251.023-.501.05-.75.082m.75-.082a24.301 24.301 0 0 1 4.5 0m0 0v5.714c0 .597.237 1.17.659 1.591L19.8 15.3M14.25 3.104c.251.023.501.05.75.082M19.8 15.3l-1.57.393A9.065 9.065 0 0 1 12 15a9.065 9.065 0 0 0-6.23-.693L5 14.5m14.8.8 1.402 1.402c1.232 1.232.65 3.318-1.067 3.611A48.309 48.309 0 0 1 12 21c-2.773 0-5.491-.235-8.135-.687-1.718-.293-2.3-2.379-1.067-3.61L5 14.5',
  category:   'M9.568 3H5.25A2.25 2.25 0 0 0 3 5.25v4.318c0 .597.237 1.17.659 1.591l9.581 9.581c.699.699 1.78.872 2.607.33a18.095 18.095 0 0 0 5.223-5.223c.542-.827.369-1.908-.33-2.607L11.16 3.66A2.25 2.25 0 0 0 9.568 3Z M6 6h.008v.008H6V6Z',
  ...
  clipboard:  'M9 5H7a2 2 0 0 0-2 2v12a2 2 0 0 0 2 2h10a2 2 0 0 0 2-2V7a2 2 0 0 0-2-2h-2M9 5a2 2 0 0 0 2 2h2a2 2 0 0 0 2-2M9 5a2 2 0 0 1 2-2h2a2 2 0 0 1 2 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01',
...
function NavItem({ to, icon, label, end = false, badge }: { to: string; icon: string; label: string; end?: boolean; badge?: React.ReactNode }) {
  return (
    <NavLink
      to={to}
      end={end}
...
export function Sidebar() {
  ...
  const isAdminOrStock = user?.roles.includes('ADMIN') || user?.roles.includes('STOCK');
  const isAdmin = user?.roles.includes('ADMIN');
  const isPedidosOrAdmin = user?.roles.includes('PEDIDOS') || user?.roles.includes('ADMIN');

  return (
    <aside className="w-64 bg-slate-950 flex flex-col h-screen fixed left-0 top-0 z-10 select-none">
      {/* Brand */}
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4 space-y-1">
        ...
        {(isAdminOrStock || isPedidosOrAdmin) && (
          <>
            <SectionLabel label="Gestión" />
            {isPedidosOrAdmin && (
              <NavItem to="/gestor-pedidos" icon={ICONS.clipboard} label="Gestión de Pedidos" />
            )}
            {isAdminOrStock && (
              <>
                <NavItem to="/admin/productos" icon={ICONS.products} label="Productos" />
                <NavItem to="/admin/ingredientes" icon={ICONS.ingredient} label="Ingredientes" />
                <NavItem to="/categorias" icon={ICONS.category} label="Categorías" />
              </>
            )}
          </>
        )}
        ...
        {isAdmin && (
          <>
            ...
            <NavItem to="/admin/usuarios" icon={ICONS.users} label="Usuarios" />
            <NavItem to="/admin" end icon={ICONS.settings} label="Configuración" />
          </>
        )}
      </nav>
