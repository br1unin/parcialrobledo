# order-manager-ui Specification

## Purpose
Gestor de Pedidos UI — a page accessible to PEDIDOS and ADMIN roles that lists all system orders with pagination and allows advancing or cancelling order states.
...
- **THEN** ve la lista de todos los pedidos con columnas: cliente, estado, total, fecha, y botones de acción

#### Scenario: Acceso denegado a CLIENT
- **WHEN** un usuario con rol CLIENT intenta acceder a `/gestor-pedidos`
- **THEN** es redirigido a `/` (acceso denegado)
